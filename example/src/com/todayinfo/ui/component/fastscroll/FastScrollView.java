...
public class FastScrollView extends FrameLayout implements OnScrollListener,
		OnTouchListener, OnHierarchyChangeListener {
	...
	private void useThumbDrawable(Drawable drawable) {
		...
		mThumbW = 31; // mCurrentThumb.getIntrinsicWidth();
		mThumbH = 50; // mCurrentThumb.getIntrinsicHeight();
		mChangedBounds = true;
	}
	...
	public void draw(Canvas canvas) {
		...
		if (mDragging && mDrawOverlay) {
			...
			canvas.drawText(mSectionText, (int) (rectF.left + rectF.right) / 2,
					(int) (rectF.bottom + rectF.top) / 2 + mOverlaySize / 4
							- descent, paint);
		} else if (alpha == 0) {
	...
	private OnScrollListener mScrollListener;// 其他效果滚动监听器
	...
	public void onChildViewAdded(View parent, View child) {
		if (child instanceof AbsListView) {
			...
			getSections();
		}else if( child instanceof  PullToRefreshListView){
			PullToRefreshListView ptr = (PullToRefreshListView) child;
			mList = (AbsListView)ptr.getRefreshableView() ;
	...
	public boolean onTouchEvent(MotionEvent me) {
		if (me.getAction() == MotionEvent.ACTION_DOWN) {
			Log.i(TAG, "me.getX()=" + me.getX() + ", getWidth()=" + getWidth()
					+ ", mThumbW=" + mThumbW);
			Log.i(TAG, "me.getY()=" + me.getY() + ", mThumbH=" + mThumbH
					+ ", mThumbY=" + mThumbY);
			...
			if (mDragging) {
				...
				} else if (mLastScrollThumbY >= 0
						&& Math.abs(mLastScrollThumbY - mThumbY) < SCROLL_TOUCH_SLOP) {
